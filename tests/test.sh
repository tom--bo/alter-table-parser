...
for i in `seq 1 $TotalCnt`; do
	# without options
	diff <(${HOME_DIR}/bin/atp < ${HOME_DIR}/tests/input/${i}.sql) ${HOME_DIR}/tests/expected/${i}.out > /dev/null 2>&1
	case $? in
...
# for i in `seq 1 $TotalCnt`; do
# 	# without options
# 	diff <(${HOME_DIR}/bin/atp -d < ${HOME_DIR}/tests/input/${i}.sql) ${HOME_DIR}/tests/expected/${i}d.out > /dev/null 2>&1
# 	case $? in
# 		0 ) echo "  case ${i} ... success" ;;
# 		1 ) echo "  case ${i} ... fail"
